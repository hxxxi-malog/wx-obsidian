...
**图片由系统自动插入**，根据原文图片在各章节中的语义关联自动匹配到对应位置。正文中无需使用 `![](url)` 标记。

### 图片插入规范（images 字段输出要求）

1. **placement 是一段原文片段**（5-15字），从 body_sections 正文中选取，图片会插入在该片段所在段落之后
2. **选择 placement 的原则**：这张图应该紧跟在哪段文字之后才能最好地辅助理解？图是对前面文字的视觉补充
3. **同一段落最多 1 张图**，不要连续插入图片
4. **不要用标题作为 placement**，用正文中的具体语句片段
